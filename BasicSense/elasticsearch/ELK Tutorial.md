...
만일 골드 이상의 라이선스를 가지고 있다면, 하나의 모니터링 클러스터와 다수ㄴ의 프로덕션 클러스터를 구성할 수 있다.
...
로그스테이시를 실행하기 전 가장 먼저 기본적인 로그스테이시 파이프라인을 실행하여 로스그스테이시 설치를 테스트 한다.

파이프라인은 입력과 출력의 두 가지 필수 요소와 필터인 선택적 요소로 구성되며, 입력 플러그인은 소스의 데이터를 사용하고, 필터 플러그인은 지정한대로 데이터를 수정하며, 출력 플러그인은 데이터를 대상으로 쓴다.
